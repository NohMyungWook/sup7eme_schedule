        ...
        <form className="template-settings-form" onSubmit={onSubmit}>
          ...
          <fieldset className="template-color-field">
            ...
            {templateColors.map((color) => {
              const isDisabled = editingTemplateId ? !canUpdate : !canCreate;
              return <label className={`template-color-swatch ${color.value} ${draft.color === color.value ? 'is-selected' : ''} ${isDisabled ? 'is-disabled' : ''}`} key={color.value} aria-label={`${color.label} 색상 선택`}><input type="radio" name="template-color" value={color.value} checked={draft.color === color.value} disabled={isDisabled} onChange={() => setDraft((current) => ({ ...current, color: color.value }))} /><span aria-hidden="true" /></label>;
            })}
