...
export function SettingsOverview({ templates, onTemplateSettingsOpen }: SettingsOverviewProps) {
      ...
      <div className="settings-overview-layout">
        <section className="settings-overview-main">
          ...
          <div className="settings-card-grid">
            ...
            <SettingsCategoryCard icon="building" title="근무지 관리" description="매장 추가, 주소, 운영 여부를 관리합니다." tags={['사당점', '석촌점', '과천점', '+5']} />
            ...
            <SettingsCategoryCard icon="shield" title="권한 및 계정" description="관리자 권한과 접근 범위를 설정합니다." tags={['역할 관리', '권한 그룹', '접근 범위']} tone="green" />
        ...
        <aside className="settings-side-panel">
          <section><h2>빠른 작업</h2><button type="button" onClick={onTemplateSettingsOpen}><span className="purple"><SettingsIcon name="clock" /></span>시간대 설정<i>›</i></button><button type="button"><span className="blue"><SettingsIcon name="building" /></span>근무지 추가<i>›</i></button><button type="button"><span className="green"><SettingsIcon name="users" /></span>직원 초대<i>›</i></button><button type="button"><span className="orange"><SettingsIcon name="bell" /></span>알림 설정<i>›</i></button></section>
