...
export function AccountManagementSettings({ stores, onBack }: AccountManagementSettingsProps) {
  const {
    accounts,
    activeTab,
    allStoresSelected,
      ...
      <div className="account-settings-layout">
        <section className="account-settings-main">
          <div className="account-settings-toolbar">
            ...
            <button className="primary" type="button" onClick={openNewAccount}>+ 관리자 초대</button>
          </div>
          ...
          <div className="account-table" role="table" aria-label="계정 목록">
            ...
            {!isLoading && filteredAccounts.map((account) => (
              ...
              >
                <span className="account-name"><i>{account.displayName.slice(0, 1)}</i><strong>{account.displayName}</strong><small>{account.username}</small></span>
        ...
        <aside className="account-editor-panel">
          <form onSubmit={submitAccount}>
            ...
            <div className="account-editor-grid">
              <label>이름<input value={draft.displayName} onChange={(event) => setDraft((current) => ({ ...current, displayName: event.target.value }))} required /></label>
              <label>아이디<input value={draft.username} onChange={(event) => setDraft((current) => ({ ...current, username: event.target.value }))} required /></label>
              <label>이메일<input value={draft.email} onChange={(event) => setDraft((current) => ({ ...current, email: event.target.value }))} placeholder="admin@kingmw.com" /></label>
              {!draft.id ? <label>초기 비밀번호<input value={draft.password ?? ''} onChange={(event) => setDraft((current) => ({ ...current, password: event.target.value }))} placeholder="미입력 시 아이디와 동일" /></label> : null}
            </div>
            <label className="account-select-field">역할<select value={draft.role} onChange={(event) => setDraft((current) => ({ ...current, role: event.target.value as AccountRole }))}><option value="manager">관리자</option><option value="viewer">조회 전용</option></select><small>{draft.role === 'manager' ? '스케줄과 설정을 수정할 수 있습니다.' : '스케줄 조회 중심 권한입니다.'}</small></label>
            <section className="account-store-field">
              <div><strong>담당 매장</strong><button type="button" className={allStoresSelected ? 'is-selected' : undefined} onClick={toggleAllStores}>전체 매장</button></div>
              <div>{stores.map((store) => <button type="button" className={draft.storeIds.includes(store.id) ? 'is-selected' : undefined} key={store.id} onClick={() => toggleStore(store.id)}>{store.name}</button>)}</div>
            ...
            <div className="account-bottom-grid">
              <section className="account-active-field">
                <strong>계정 상태</strong>
                <button type="button" className={`account-status-toggle ${draft.status === 'active' ? 'is-on' : ''}`} onClick={() => setDraft((current) => ({ ...current, status: current.status === 'active' ? 'inactive' : 'active' }))}><i />{draft.status === 'active' ? '활성' : '비활성'}</button>
            ...
            <div className="account-editor-actions"><button type="button" onClick={resetDraft}>취소</button><button className="primary" type="submit">변경 저장</button></div>
          </form>
        </aside>
      </div>
    </>
