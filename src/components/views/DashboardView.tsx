...
export function DashboardView({ storeId, stores, month, employees, shifts, onStoreChange, onMonthChange, onDateSelect }: DashboardViewProps) {
  ...
  return <>
    ...
    <section className="dashboard-panel calendar-panel"><div className="dashboard-panel-title"><div><h2>{month.replace('-', '년 ')}월 운영 현황</h2><p>날짜를 누르면 해당 주 스케줄로 이동합니다.</p></div></div><div className="month-calendar">{weekdays.map((weekday) => <strong className="month-weekday" key={weekday}>{weekday}</strong>)}{Array.from({ length: toDate(days[0]).getDay() }, (_, index) => <span className="calendar-blank" key={`blank-${index}`} />)}{days.map((date) => { const dayGaps = gapsByDate.get(date) ?? []; const dayShifts = storeShifts.filter((shift) => shift.date === date); const dayEmployeeCount = new Set(dayShifts.map((shift) => shift.employeeId)).size; return <button className={`calendar-day ${dayGaps.length ? 'has-gap' : 'is-covered'}`} type="button" key={date} onClick={() => onDateSelect(date)}><span>{toDate(date).getDate()}</span><small>{dayEmployeeCount}명 · {dayShifts.length}건</small>{dayGaps.length ? <em>인원 부족 {dayGaps.length}구간</em> : data?.hasCoverageRules ? <em>규칙 충족</em> : <em>필요 인원 미설정</em>}</button>; })}</div></section>
  </>;
}
