...
export function NotesView(props: NotesViewProps) {
  const { notes, filteredNotes, storeFilter, memoStoreId, memoDate, memoText, editingMemoKey, isManager } = props;
  return (
    <>
      ...
      <div className="memo-management-layout">
        <section className="memo-list" aria-label="특이사항 목록">
          {filteredNotes.map((note) => (
            <article className="memo-item" key={`${note.storeId}:${note.date}`}>
              <header><div><span>{getStoreName(note.storeId)}</span><strong>{fullDateLabel(note.date)}</strong></div>
                {isManager ? <div className="memo-item-actions"><button type="button" onClick={() => props.onEdit(note)}>수정</button><button className="danger" type="button" onClick={() => props.onDelete(note)}>삭제</button></div> : null}
              </header>
              <p>{note.text}</p>
            </article>
          ))}
        ...
        {isManager ? (
          <form className="memo-editor-panel" onSubmit={props.onSubmit}>
            ...
            <label>매장<StoreSelect value={memoStoreId} onChange={props.onMemoStoreChange} /></label>
            ...
            <div className="form-actions"><button type="button" onClick={props.onReset}>초기화</button><button className="primary" type="submit">{editingMemoKey ? '변경 저장' : '메모 등록'}</button></div>
          </form>
        ) : null}
      </div>
    </>
